...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (response.data.message === 'Login successful') {
        ...
        setTest(response.data.assigned_test)
        console.log(test)
         // Imposta l'utente
  ...
  const signup = async (username, email, password) => {
    ...
    try {
      ...
      if (response.data.message === 'User created successfully') {
        setUser(response.data.user_id);
        setUserId(response.data.userId); // Imposta l'utente
        setIsAuthenticated(true);  // L'utente è autenticato
        try {
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      if (response.data.logged_in) {
        setUser(response.data.user_id)
        setUserId(response.data.userId);
        setTest(response.data.assigned_test)  // Imposta l'utente
        console.log(test)
        setIsAuthenticated(true);  // L'utente è autenticato
        navigate('/home');  // Reindirizza alla home page
      } else {
  ...
  return (
    <AuthContext.Provider value={{ user,userId,test, loading, error, isAuthenticated, login, signup, logout, checkAuthStatus }}>
      {children}
