...
def login():
    ...
    return jsonify({'message': 'Login successful', 'user_id': user.id, 'userId':user.userId, 'assigned_test':user.assigned_test, 'next': next_page}), 200
...
def status():
    ...
    if current_user.is_authenticated:
        return jsonify({'logged_in': True, 'user_id': current_user.id, 'userId':current_user.userId, 'assigned_test':current_user.assigned_test}), 200
    else:
